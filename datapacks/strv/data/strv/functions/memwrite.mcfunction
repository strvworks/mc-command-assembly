...
# 書き込み
execute as @e[tag=mem_reader,limit=1] at @s store result block ~ ~ ~ SuccessCount int 1 run scoreboard players get @e[tag=mem_controller,limit=1] mem_write_value
 
scoreboard players set @e[tag=mem_controller] mem_write_state 0
tellraw @a ["",{"text":"select mem:"},{"score":{"name":"@e[tag=mem_controller]","objective":"mem_select"}}," ",{"text":"write velue:"},{"score":{"name":"@e[tag=mem_controller]","objective":"mem_write_value"}}]
